...
const HOLIDAY_SERVICE_SLUG = "holiday-packages";

const FALLBACK = {
  title: "Choose Your Preferred Umrah Package",
  subtitle:
    "Selecting your preferred Umrah package will allow us to tailor your experience! You can also change your preferences at any time from your profile.",
};

export function PackageList() {
  const { data: service, isLoading } = useServiceDetailQuery(HOLIDAY_SERVICE_SLUG);

  if (isLoading) return null;

  const title = service?.title || FALLBACK.title;
  const subtitle = service?.subtitle || FALLBACK.subtitle;
      ...
      <section className="bg-[#FEFBF5] pb-14 tablet:pb-20">
        <Container>
          ...
          <div className="flex flex-wrap items-center gap-2 border-b border-[#e9e5dc] py-4">
            ...
            {(
              [
                ["Category", "All", ["All", "Hajj", "Umrah"]],
                ["Price", "Any", ["Any", "Under ৳10,000", "৳10,000+"]],
                ["Nights", "Any", ["Any", "5 Nights", "10 Nights"]],
                ["Start Date", "Select", ["Select", "2026-07-24", "2026-08-03"]],
                ["End Date", "Select", ["Select", "2026-08-13", "2026-09-03"]],
              ] as const
            ).map(([label, value, options]) => (
              <div
                key={label}
                className="flex h-[46px] w-[160px] items-center justify-between rounded-[8px] border-[0.5px] border-gray-900/40 bg-transparent px-3 py-2 text-xs text-gray-900"
              >
                <span>{label}</span>
                <Select defaultValue={value.toLowerCase()}>
                  ...
                  <SelectContent
                    align="start"
                    alignItemWithTrigger={false}
                    className="min-w-[100px] rounded border border-[#d8d3c8] bg-white p-1 shadow-md ring-0"
                  >
                    {options.map((option) => (
          ...
          <div className="grid w-full gap-4 py-5 tablet:grid-cols-2 tablet:gap-5 desktop:grid-cols-3 desktop:gap-6">
            {packageList.map((packageData) => (
              <PackageCard key={packageData.title} package={packageData} />
            ))}
          </div>
          <nav
          ...
          >
            ...
            <button type="button" aria-label="Previous page" className="text-lg">
              ‹
            </button>
            {[1, 2, 3, 4, 5].map((page) => (
              <button
                key={page}
                type="button"
                className={page === 1 ? "font-semibold text-primary" : ""}
              >
                {page}
              </button>
            ))}
            <button type="button" aria-label="Next page" className="text-lg">
              ›
            </button>
            <button type="button" className="text-neutral-900">
              Last
