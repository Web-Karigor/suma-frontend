...
export function PackageCard({ package: packageData }: { package: PackageListItem }) {
  return (
    <article className="group relative h-[472px] min-w-0 overflow-hidden rounded-[18px] border border-[#ddd8cb] bg-white shadow-[0_2px_8px_rgb(10_12_12/8%)]">
      ...
      <div className="absolute right-2 bottom-2 left-2 flex h-[222px] flex-col gap-2.5 rounded-[10px] bg-overlay-white-84 p-4 backdrop-blur-md">
        <Link href={packageData.href} aria-label={`View ${packageData.title}`} className="group absolute top-4 right-4 inline-flex size-10 items-center justify-center overflow-hidden rounded-[10px] bg-gray-50 text-neutral-950 transition-colors duration-200 hover:bg-primary hover:text-white">
        ...
        {packageData.packageType ? <p className="truncate text-[15px] text-neutral-800">{packageData.packageType.name}</p> : null}
        {packageData.date ? <div className="flex items-center gap-2 text-xs text-neutral-950"><CalendarDays className="size-5 shrink-0" /><span>{packageData.date}</span></div> : null}
        {(packageData.packageType || packageData.nights) ? <div className="flex items-center gap-8 text-xs text-neutral-950">{packageData.packageType ? <span className="flex items-center gap-2"><StarIcon className="size-5 shrink-0" />{packageData.packageType.name}</span> : null}{packageData.nights ? <span className="flex items-center gap-2"><Moon className="size-5 shrink-0" />{packageData.nights}</span> : null}</div> : null}
        <div className="mt-auto flex items-center justify-between gap-2"><p className="text-xl font-semibold text-primary tablet:text-3xl">৳ {packageData.price.toLocaleString()}</p><div className="flex items-center gap-[21px]"><Link href={packageData.href} className="inline-flex h-[49px] w-[140px] items-center justify-center rounded-button bg-primary px-3 text-sm font-medium text-white">Book Now</Link><button type="button" aria-label="Share package" className="inline-flex size-12 shrink-0 items-center justify-center rounded-[12px] border-[0.5px] border-teal-600 bg-gray-50 text-primary"><ShareIcon className="size-6" /></button></div></div>
      </div>
    </article>
  );
}
