...
export function VisaHero() {
  return (
    <section className="relative flex min-h-[380px] items-end overflow-hidden bg-neutral-900 pb-16 tablet:min-h-[470px] tablet:pb-20 desktop:min-h-[580px] desktop:pb-24">
      <Image
        src={heroImage}
        alt="Travel documents and journey planning"
        fill
        priority
      ...
      <Container className="relative z-10 text-center text-white">
        ...
        <h1 className="mt-1 text-4xl font-semibold tablet:text-6xl">
          Visa Assistance
        </h1>

        <p className="mx-auto mt-3 max-w-[620px] text-xs leading-[1.5] tablet:text-sm">
          Start browsing patterns where a ShareTrip Visa Guide. Document
          checklist, processing time, and inquiry document form.
        </p>
        ...
        <div className="mx-auto mt-8 flex max-w-[900px] flex-col gap-3 rounded-[12px] border-l-4 border-primary bg-white p-2 text-left tablet:flex-row tablet:items-center tablet:gap-4 tablet:p-3">
          ...
          <Select defaultValue="thailand">
            <SelectTrigger
              showCloseIcon
              className="h-[49px] w-full rounded-md border-0 bg-gray-100 px-4 text-xs text-neutral-700 shadow-none tablet:w-[270px]"
            >
              <SelectValue />
            </SelectTrigger>
          ...
          <Link
            href="/contact"
            className="inline-flex h-[49px] items-center justify-center rounded-md bg-primary px-8 text-xs font-medium text-white transition-colors hover:bg-primary-700"
          >
            View
          </Link>
        </div>
