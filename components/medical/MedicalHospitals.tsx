...
import type { ServiceHospital } from "@/types/service-detail";

import "swiper/css";

const VISIBLE_COUNT = 5;

function HospitalCard({ hospital }: { hospital: ServiceHospital }) {
  return (
    <article className="flex h-full flex-col rounded-2xl border border-gray-200 bg-teal-50 p-5 desktop-xl:h-[570px] desktop-xl:w-[564px]">
      ...
      <div className="flex flex-1 flex-col pt-4">
        ...
        {hospital.specialties.length > 0 ? (
          <p className="mt-3 text-[14px] leading-[1.5] text-neutral-700">
...
function HospitalsSlider({ hospitals }: { hospitals: ServiceHospital[] }) {
  return (
    <Swiper
      speed={600}
      grabCursor
...
export function MedicalHospitals({
  ...
  const needsSlider = hospitals.length > VISIBLE_COUNT;

  return (
    <section className="bg-teal-50 py-12 tablet:py-16 desktop-xl:py-[80px]">
      <Container>
        <div className="flex flex-col gap-8">
          ...
          {needsSlider ? (
            <HospitalsSlider hospitals={hospitals} />
          ) : (
