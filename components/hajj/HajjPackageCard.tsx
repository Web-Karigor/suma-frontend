  ...
  return (
    <article className="relative mx-auto min-h-0 min-w-0 w-full max-w-[858px] overflow-visible rounded-[14px] transition-transform duration-300 hover:-translate-y-1 desktop:h-[1128px]">
      ...
      <div className="relative z-10 flex flex-col gap-4 rounded-[24px] bg-white px-4 pb-4 tablet:px-5 tablet:pb-5 desktop:absolute desktop:top-[476px] desktop:left-1/2 desktop:h-[652px] desktop:w-[calc(100%_-_58px)] desktop:max-w-[800px] desktop:-translate-x-1/2 desktop:px-8 desktop:pb-8">
        <h2
          className="-mx-1 -mt-5 relative flex h-[84px] w-full items-center justify-center rounded-[12px] px-8 text-center text-[28px] leading-[128%] font-semibold text-primary backdrop-blur-[32px] desktop:-mx-8 desktop:mt-0 desktop:w-[calc(100%_+_64px)] desktop:max-w-[800px]"
          style={{
            background:
        ...
        {packageData.highlights.length > 0 ? (
          <>
            <p className="mt-2 flex h-[25px] items-center text-base leading-[158%] font-semibold uppercase text-[#8E8E8E]">
              Service Highlights
            </p>
            <ul className="flex flex-col gap-3">
              {packageData.highlights.map((highlight, index) => (
                <li
                  key={`${highlight}-${index}`}
                  className="flex min-h-[26px] items-center gap-1.5 text-base leading-[160%] text-[#005655]"
                >
        ...
        {packageData.price ? (
          <p className="mt-2 text-[22px] font-semibold text-[#00605F]">
