  ...
  return (
    <div className="flex flex-col gap-6 tablet:flex-row tablet:gap-8">
      {[left, right].map((column, columnIndex) => (
        <div
          key={columnIndex}
          className="flex min-w-0 flex-1 flex-col gap-8 desktop-xl:w-[200px] desktop-xl:flex-none"
        >
          {column.map((item) => (
            <div key={item} className="flex items-center gap-3">
              ...
              <span
                className={`text-base leading-[1.6] font-normal ${textClass}`}
              >
                {item}
              </span>
            </div>
...
export function CorporateServices({ services }: CorporateServicesProps) {
  ...
  return (
    <section
      id="services"
      className="scroll-mt-28 bg-teal-950 pt-12 tablet:pt-16 desktop-xl:pt-[117px]"
    >
